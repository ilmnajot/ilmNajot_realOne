...
public class AuthServiceImpl implements AuthService {
    ...
    public ApiResponse register(UserRequest form) {
        Optional<User> userByEmail = userRepository.findByUsername(form.getUsername());
        if (userByEmail.isPresent()) {
            throw new UserException("user with email " + form.getUsername() + " already exists", HttpStatus.CONFLICT);
        }
